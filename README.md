...
- 상품 제목 부분 검색 및 영문 대소문자 무시
- 검색어 생략·빈 값·공백 입력 시 전체 조회
- 검색 파라미터 전달 및 DB 검색 통합 테스트

## API

| Method | Path | 기능 | 성공 응답 |
|---|---|---|---|
| GET | /api/products | 상품 목록 조회 및 선택적 keyword 검색 | 200 |
...
- 제목에 검색어가 포함된 상품을 조회합니다.
- 영문 대소문자를 구분하지 않습니다.
- 검색어 앞뒤 공백을 제거합니다.
- 검색어가 없거나 비어 있으면 전체 상품을 조회합니다.
- 거래 지역은 검색 대상이 아닙니다.
- 결과는 상품 ID 오름차순입니다.
- 검색 결과가 없으면 200과 빈 배열을 반환합니다.
- `%`와 `_`는 일반 문자로 검색합니다.

UTF-8로 URL 인코딩한 "키보드" 검색 예시:

```bash
curl -i 'http://localhost:8080/api/products?keyword=%ED%82%A4%EB%B3%B4%EB%93%9C'
```
...
- 로그인, 이미지 업로드, 채팅은 아직 구현하지 않았습니다.
- 상품 목록과 검색은 페이지네이션 없이 전체 결과를 반환합니다.
